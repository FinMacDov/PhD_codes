...
#        jet_time = jet_time*TIME_CORRECTION_FACTOR*dt
...
#        if t_max < max(data_x): t_max = max(data_x)
        if np.isnan(sum(big_data_set['dfs'][idx]['height [Mm]']))==True:
            ...
            if save_pd_cvs==True:
                ...
                orig_data.to_csv('original_data_data.csv', index = True, header=True)
...
#            detrended_jet_width = selected_jet_width-selected_jet_width.mean()
            # get riase abnd fall
            max_h_idx = big_data_set['dfs'][0]['Height [Mm]'].dropna().argmax()
#            max_h_time = big_data_set['dfs'][0]['time [s]'].dropna()[max_h_idx]*TIME_CORRECTION_FACTOR*dt
            max_h_time = big_data_set['dfs'][0]['time [s]'].dropna()[max_h_idx]*TIME_CORRECTION_FACTOR
            ...
            coefs_rise = poly.polyfit(rise_time,rise_widths,2)
            ...
            coefs_fall = poly.polyfit(fall_time,fall_widths,2)
            ...
            rolling_rise_mean = rise_panda.detrend.rolling(window=15).mean()
            rolling_fall_mean = fall_panda.detrend.rolling(window=10).mean()
            ...
            if save_pd_cvs==True:
                rise_panda.to_csv('detrended_rise_data.csv', index = True, header=True)
                fall_panda.to_csv('detrended_fall_data.csv', index = True, header=True)

                rolling_rise_mean.to_csv('rise_data.csv', index = True, header=True)
                rolling_fall_mean.to_csv('fall_data.csv', index = True, header=True)            
            if testing == True:
                fig, ax = plt.subplots(figsize=(20,12))
