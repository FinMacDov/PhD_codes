...
code_root = path_2_shared_drive+'/tilt_python/sharc_run/yt_images/'
# 2D
#path_root = '/j/2D/'
#
#driver_time = ['P200','P300']
##driver_time = ['P300']
#mag_str = ['B50']
##Amp = ['A20','A40','A60','A80']
#Amp = ['A40']
...
mag_str = ['B60']
Amp = ['A60']
tilt = ['T0']#, 'T5', 'T10', 'T15', 'T20', 'T25',
       # 'T30', 'T35', 'T40', 'T45', 'T50', 'T55', 'T60']


path_root = path_2_shared_drive+'/T/'

sim_nbs = len(tilt)*len(driver_time)*len(mag_str)*len(Amp)


path_2_files = np.empty(sim_nbs, dtype='U120')
file_names = np.empty(sim_nbs, dtype='U120')
# vpn timed out 
n = 0
path_2_files = [os.getenv("given_path")]
file_names = [os.getenv("given_jet_name")]
...
#             'phase_plot_of_Density', 'Rolling_difference_of_Density']
#cmaps = [ 'coolwarm','seismic', 'gist_heat', 'Greens','Greys','Greys']
#
#var_names = ['phase_plot_of_Density', 'running_difference_of_Density']
#cmaps = ['Greys','seismic']
...
for stuff in range(len(file_names)):
    ...
    Full_path = os.path.join(path_2_file, file_name+'_')
    print(Full_path, path_2_file, file_name)

    for ind, ti in enumerate(time_stamps):
        jet_sides_index1_array = []
        jet_sides_index2_array = []
        offset = str(ti).zfill(4)
        save_folder = os.path.join(code_root+file_name,'t_'+offset)
...
#            var_b2_data = ad['b2']*unit_magenticfield
        ...
        if vtu_file==True:
            ...
            if first_step == True:
                ds0 = apt.load_vtkfile(0, file=Full_path, type='vtu')
                var_rho0_data = apt.vtkfiles.rgplot(ds0.rho, data=ds0, cmap='hot')
                var_rho0_data, x_grid0, y_grid0 = var_rho0_data.get_data(xres=xres, yres=yres)
                del ds0

            var_rho_data = apt.vtkfiles.rgplot(ds.rho, data=ds, cmap='hot')
            var_rho_data, x_grid0, y_grid0 = var_rho_data.get_data(xres=xres, yres=yres)
#                arr_rho0 = np.zeros((var_rho_data.shape[0], var_rho_data.shape[1], 1))
#                arr_rho0[:, :, 0] = var_rho0_data
...
#        arr_phase_rho0 = np.zeros((var_rho_data.shape[0], var_rho_data.shape[1], 1))
#        arr_phase_rho = np.zeros((var_rho_data.shape[0], var_rho_data.shape[1], 1))
#        arr_run_avg_rho = np.zeros((var_rho_data.shape[0], var_rho_data.shape[1], 1))
...
#        arr_phase_rho0[:, :, 0] = var_rho0_data
#        rho_phase = np.subtract(var_rho_data,
#                                var_rho0_data)
#        rho_phase = var_rho0_data        
#        arr_phase_rho[:, :, 0] = rho_phase/unit_density

        if first_step == True:  
            rho_tn1 = arr_rho
            first_step = False
#        else:
#            # need to -1 fromt i when plotting
#            # This methods aviods having to load 2 data sets 
##            arr_run_avg_rho[:, :, 0] = abs(np.reshape(rho_tn1,np.shape(var_rho_data))-var_rho_data)
#            # saves current time step
#            rho_tn1 = arr_rho
...
#        temperature=(arr_T,"K"), phase_plot_of_Density=(arr_b1, "dimensionless"),
#        Rolling_difference_of_Density=(arr_run_avg_rho, "g/cm**3"))

        data = dict(density=(arr_rho, "g/cm**3"))

#        data = dict(density=(arr_rho, "g/cm**3"), b1=(arr_b1, "gauss"),
#        b2=(arr_b2, "gauss"),pressure=(arr_p,"dyne/cm**2"),
#        temperature=(arr_T,"K"), rho0=(arr_phase_rho0,"g/cm**3"),
#        rho1=(rho_tn1, "g/cm**3"))
    ...
    #        print(i)
   #  ds.r['stream','phase_plot_of_Density'] # how to print out array
...
#        b2_grad_fields = ds.add_gradient_fields(('stream', 'b2'))          
        ...
        def _phase_data_(field, data):
            rho0 = data['rho0']
            rho1 = data['density']
 #           test = rho1-rho0
 #           test1 = abs(test)
#            print('range: min ', np.min(test), ', max: ', np.max(test),' abs range: min ', np.min(test1), ' max ',np.max(test1))
            return rho1-rho0
            
        def _Rolling_difference_of_Density_(field, data):
            return data['density']-data['rho1']
...
#                     sampling_type="cell")    
#
#        ds.add_field(('stream','phase_plot_of_Density'), function=_phase_data_, units="g/cm**3", take_log=False,
#                     sampling_type="cell")
#        
#        ds.add_field(('stream','running_difference_of_Density'), function=_Rolling_difference_of_Density_, units="g/cm**3", take_log=False,
#                     sampling_type="cell")
...
#            rho_tn1 = arr_rho
        ...
        for idx, var_name in enumerate(var_names):
            ...
            slc = yt.SlicePlot(ds, "z", [var_name], center=[0.0, y_limit/2+shift_factor, 0],
                               ...
                               origin=(0, 0, 'domain'), fontsize=58)#fontsize=52)
...
#            slc.set_axes_unit("m")
            ...
            if var_name == 'pressure':
                slc.set_zlim('pressure', 1.7e7*dyne_cm2_to_Pa, 1e10*dyne_cm2_to_Pa)
    ...
    #                slc.set_zlim('mag_tension_x', -2e-5, 2e-5)
                    ...
                    slc.set_unit(var_name, 'Pa/m')
            if var_name == 'phase_plot_of_Density':
                slc.set_unit(var_name, 'kg/m**3')
                ds.current_time = yt.units.yt_array.YTQuantity(ti*dt*second)
                slc.set_zlim(var_name, -2e-8, 1.4e-7)

            if var_name == 'running_difference_of_Density':
#                slc.set_zlim('temperature', 8e3,2e6)
                slc.set_unit(var_name, 'kg/m**3')
                ds.current_time = yt.units.yt_array.YTQuantity((ti-1)*dt*second)
                slc.set_zlim(var_name, -2.5e-8, 2.5e-8)
