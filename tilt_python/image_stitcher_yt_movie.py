...
jet_save_name = jet_name+'T_'+tilt_nbs
nb_of_images_per_plot = 4
img_root_folder = path_2_shared_drive + '/j/tilt_python/sharc_run/yt_images/'
ti_number = np.arange(0,131)
#ti_number = np.arange(0,3)
save_dir = img_root_folder+'figs_for_pres/'+jet_save_name+'/'
Path_creator(save_dir).mkdir(parents=True, exist_ok=True)
for time in ti_number:    
    list_of_dirs = []
    for name_path in name:
        dir_2_image = glob.glob(img_root_folder+name_path+'/t_'+str(time).zfill(4)+'/*.png')
