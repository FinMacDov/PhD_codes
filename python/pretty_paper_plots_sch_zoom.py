...
def add_inner_title(ax, title, loc, **kwargs):
    ...
    prop = dict(path_effects=[withStroke(foreground='black', linewidth=2.5)],
                size=plt.rcParams['legend.fontsize']+4)
...
cmaps = ['gray']
labs = ["Numerical Schlieren"]
clipped_rho_data = []
clipped_sch = []
clipped_Te_data = []
full_time_stamps = np.linspace(t0, t1, nb_steps, dtype=int)
time_stamps = np.asarray((full_time_stamps[52],full_time_stamps[67]))
...
norm = mplt_col.Normalize(vmax=schmax, vmin=schmin)
...
F.savefig('sch_zoom.png', bbox_inches='tight')
