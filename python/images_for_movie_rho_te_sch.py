...
#                FIRST = False
        ...
        #    print(new_dex, dex_num,)
            im = ax.imshow(z, norm=norm[dex_num], cmap=cmaps[dex_num],
            ...
            grid2[dex_num].cax.colorbar(im_list[dex_num])
            cax = grid2.cbar_axes[dex_num]
            axis = cax.axis[cax.orientation]
            axis.label.set_text(labs[dex_num])
            if dex_num == 0:
                ax.set_title('t = '+str(physical_time[ind])+' s', )
            dex_num += 1
        F.savefig('sj_paper/movie/rho_Te_sch/sj_image_for_movie_'+ offset +'.png', bbox_inches='tight')
        F.clf()
plt.close()
